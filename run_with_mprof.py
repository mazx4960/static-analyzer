...
if args.command:
    ...
    result = subprocess.run(['python3', '-m', 'mprof', 'run', '-T', str(args.interval), '-C'] + command,
                            stdout=subprocess.DEVNULL)
    peak_memory_log = subprocess.run(['python3', '-m', 'mprof', 'peak'], capture_output=True, text=True)
