...
TEST(ExtractorTest, TestUses) {
  ...
  auto const op = [&relationships](Node *node) { RelationshipExtractor::ExtractUses(relationships, node); };
  program->VisitAll(op);
  ...
  std::vector<Relationship *> expected = {
      ...
      new UsesRelationship(new AssignEntity(0), new VariableEntity("v2")),
  };
