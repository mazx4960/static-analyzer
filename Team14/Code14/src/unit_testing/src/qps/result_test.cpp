...
struct ResultTestStatics {
 public:
  ...
  inline static std::vector<QuerySynonym *> syn_vec_1_ =
      {new QuerySynonym("var_x", EntityType::kVariable), new QuerySynonym("assign_y", EntityType::kAssignStmt),
       new QuerySynonym("proc_z", EntityType::kProcedure),};

  inline static std::vector<ElemReference *> elemref_vec_var_ = {new SynonymReference(syn_vec_1_[0]),};
  inline static std::vector<ElemReference *> elemref_vec_full_ =
      {new SynonymReference(syn_vec_1_[0]), new SynonymReference(syn_vec_1_[1]), new SynonymReference(syn_vec_1_[2])};

  inline static std::vector<ElemReference *> elemref_vec_full_shuffled_ =
      {new SynonymReference(syn_vec_1_[1]), new SynonymReference(syn_vec_1_[2]), new SynonymReference(syn_vec_1_[0])};

  inline static ResultRow row1_ = {{new QuerySynonym("var_x", EntityType::kVariable), new VariableEntity("a")},
                                   {new QuerySynonym("assign_y", EntityType::kAssignStmt), new AssignStmtEntity("1")},
                                   {new QuerySynonym("proc_z", EntityType::kProcedure), new ProcedureEntity("main")}};

  inline static ResultRow row2_ = {{new QuerySynonym("var_x", EntityType::kVariable), new VariableEntity("b")},
                                   {new QuerySynonym("assign_y", EntityType::kAssignStmt), new AssignStmtEntity("2")},
                                   {new QuerySynonym("proc_z", EntityType::kProcedure),
                                    new ProcedureEntity("do_that")}};

  inline static ResultRow row3_ = {{new QuerySynonym("var_x", EntityType::kVariable), new VariableEntity("a")},
...
TEST(ElemResultCreationTest, AllSynonymShuffledFullTable) {
  ...
  std::unordered_set<std::string> exp_set = {"1 main a", "2 do_that b", "3 do_this a"};
