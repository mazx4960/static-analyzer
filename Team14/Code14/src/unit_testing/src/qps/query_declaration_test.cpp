...
TEST(QueryDeclarationPointerEqualityTest, EqualStatement) {
  ...
  std::unordered_set<QueryReference *, QueryDeclarationHashFunction, QueryDeclarationPointerEquality> s = {q1, q2};
  ASSERT_EQ(s.size(), 1);
  ASSERT_TRUE((*q1) == (*q2));
}
...
TEST(QueryDeclarationPointerEqualityTest, EqualWildCard) {
  auto *q1 = new WildcardReference();
  auto *q2 = new WildcardReference();
  std::unordered_set<QueryReference *, QueryDeclarationHashFunction, QueryDeclarationPointerEquality> s = {q1, q2};
  ASSERT_EQ(s.size(), 1);
  ASSERT_TRUE((*q1) == (*q2));
}
...
TEST(QueryDeclarationPointerEqualityTest, EqualString) {
  auto *q1 = new IdentReference("str");
  auto *q2 = new IdentReference("str");
  std::unordered_set<QueryReference *, QueryDeclarationHashFunction, QueryDeclarationPointerEquality> s = {q1, q2};
  ASSERT_EQ(s.size(), 1);
  ASSERT_TRUE((*q1) == (*q2));
}
TEST(QueryDeclarationPointerEqualityTest, EqualInteger) {
  auto *q1 = new IntegerReference("123");
  auto *q2 = new IntegerReference("123");
  std::unordered_set<QueryReference *, QueryDeclarationHashFunction, QueryDeclarationPointerEquality> s = {q1, q2};
  ASSERT_EQ(s.size(), 1);
  ASSERT_TRUE((*q1) == (*q2));
}