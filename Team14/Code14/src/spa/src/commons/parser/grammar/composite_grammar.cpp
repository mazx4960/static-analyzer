...
Node* CompositeGrammarRule::parseNode(TokenIterator& tokenStream) {
  ...
  for (auto* rule_fragment : rule_fragments_) { rule_fragment->parseStream(tokenStream, child_nodes); }
...
Node* ParenthesizedGrammarRule::assembleNode(std::vector<Node*> children) { return children[0]; }
