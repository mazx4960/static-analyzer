...
void SimpleParser::ValidateProgram(Node *node) {
  ...
  if (HasDuplicates(&procedure_list)) { throw ParseSemanticError("Program contains procedures with the same name"); }
  if (HasInvalidCalls(&procedures, &calls)) {
    throw ParseSemanticError("Program contains calls to non-existent procedures");
  }
  if (IsCyclic(&call_map)) { throw ParseSemanticError("Cyclic call detected"); }
}
bool SimpleParser::HasDuplicates(std::vector<std::string> *procedures) {
  std::unordered_set<std::string> set;
  for (const auto &proc : *procedures) {
    if (set.find(proc) != set.end()) { return true; }
    set.insert(proc);
  }
  return false;
}
bool SimpleParser::HasInvalidCalls(StringSet *procedures, StringSet *calls) {
  for (const auto &call : *calls) {
    if (procedures->find(call) == procedures->end()) { return true; }
  }
  return false;
}
...
bool SimpleParser::IsCyclicUtil(const std::string &proc, StringMap *call_map, StringSet *visited,
                                StringSet *rec_stack) {
  visited->insert(proc);
